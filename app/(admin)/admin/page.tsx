...
function StatCard({ icon, label, value, color }: { icon: string; label: string; value: string; color: string }) {
  return (
    <div className="bg-white rounded-2xl p-5 border border-outline-variant/30 flex items-center gap-4">
      ...
      <div>
        <p className="text-[13px] text-outline">{label}</p>
        <p className="text-[24px] font-extrabold text-on-surface">{value}</p>
      </div>
    </div>
  );
}
...
function FunnelBar({ label, count, total, color }: { label: string; count: number; total: number; color: string }) {
  ...
  return (
    <div className="flex flex-col gap-1.5">
      ...
      <div className="h-7 w-full bg-surface-container-low rounded-lg overflow-hidden">
...
export default async function AdminDashboard() {
  ...
  return (
    <div className="flex flex-col gap-8">
      <h1 className="text-[22px] font-extrabold text-on-surface">داشبورد آمار</h1>

      {/* آمار کلی */}
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <StatCard icon="group" label="کل کاربران" value={s.totalUsers.toLocaleString("fa-IR")} color="var(--color-primary)" />
        <StatCard icon="person_add" label="ثبت‌نام امروز" value={s.newToday.toLocaleString("fa-IR")} color="var(--color-secondary)" />
        <StatCard icon="bolt" label="فعال ۲۴ ساعت" value={s.active24h.toLocaleString("fa-IR")} color="var(--color-tertiary-container)" />
        <StatCard icon="calendar_month" label="فعال ۷ روز" value={s.active7d.toLocaleString("fa-IR")} color="var(--color-tertiary)" />
      </div>
      ...
      <section className="bg-white rounded-2xl p-6 border border-outline-variant/30">
        ...
        {/* نرخ ریزش بین مراحل */}
        <div className="grid grid-cols-3 gap-3 mt-6 pt-5 border-t border-outline-variant/30">
          <div className="text-center">
            <p className="text-[12px] text-outline">ریزش مرحله ۱→۲</p>
            <p className="text-[20px] font-bold text-error">{churn(registered, day1Done).toLocaleString("fa-IR")}٪</p>
          </div>
          <div className="text-center">
            <p className="text-[12px] text-outline">ریزش مرحله ۲→۳</p>
            <p className="text-[20px] font-bold text-error">{churn(day1Done, leadDone).toLocaleString("fa-IR")}٪</p>
          </div>
          <div className="text-center">
            <p className="text-[12px] text-outline">ریزش مرحله ۳→۴</p>
            <p className="text-[20px] font-bold text-error">{churn(leadDone, week2).toLocaleString("fa-IR")}٪</p>
          </div>
        </div>
      </section>

      <p className="text-[12px] text-outline text-center">مجموع جلسات مطالعه ثبت‌شده: {s.totalSessions.toLocaleString("fa-IR")}</p>
    </div>
  );
}
